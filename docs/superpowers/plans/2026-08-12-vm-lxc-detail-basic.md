...
2. **`src/widgets/proxmoxvm/component.jsx` (the only caller of that route) isn't registered in `src/widgets/widgets.js` at all** — confirmed via grep, it doesn't appear in the widget-type registry the rest of the app uses to resolve a `widget.type` string to a component. It's unreachable dead code.

Net effect: this specific route+widget pairing has never worked against this fork's real config and isn't wired into the dashboard regardless. This plan does not touch either file — it adds a fresh, correct config accessor (`getPveConfig()`, mirroring the already-correct `getSmartConfig()` pattern) and a new route, rather than repairing dead code outside its scope. Worth a decision from Pavel later: delete the dead route+widget, or fix and register them for other forks that might configure Proxmox differently.
